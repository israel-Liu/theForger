---
title: TypeScript
date: 2019-08-04 18:17:01
tags: language
---
# [TypeScript Language Learning Note](https://github.com/Microsoft/TypeScript/blob/master/doc/spec.md)

# <a name="1"/>1 Introduction

设计用来编写大型程序, 支持类型系统, 使用类型推断。可以直接翻译为 javascript 语言运行。可以使用V8高效运行.
支持模块化, 编译器选项可以设置模块静态或者动态加载。类型系统可以用来支持类型安全，检查错误。
...
通过使用 TypeScript language service find the members of a string object.

&emsp;&emsp;![](images/image1.png)

通过 providing type annotations. 用来检查参数正确性.
...
生成的js将不会包含这些信息, the TypeScript compiler will emit the following JavaScript code:
...
In general, TypeScript erases all type information before emitting JavaScript.(所以就没有类型检查了)

## <a name="1.1"/>1.1 Ambient Declarations

TypeScript 变量在使用前要声明，如果确定生成的 js 使用的变量会被提供，在TypeScript里面进行声明。
...
TypeScript by default includes a file 'lib.d.ts' that provides interface declarations for the built-in JavaScript library.

A programmer could supply a declaration such as:
...
支持 closures 可以用来捕获变量，定义范围，可以作为其它程序的异步回调 handle。

TypeScript 函数支持 *signature* . 可以在参数中定义回调函数类型.
...
类似C++的大括号用来构建对象， *object types* 用来声明期望的对象行为. 
...
可以命名对象类型通过使用 *interfaces*. 类似C++的class, 不用后面的分号.
...
Object types are compared *structurally*. 就是成员布局相同，互相兼容.
...
推断不出来的就算 'any'，可以通过签名中推断，叫"top-down"。
...
Contextual typing is also useful 可以提供自动补全.

Section [4.23](#4.23) provides additional information about contextually typed expressions.

## <a name="1.6"/>1.6 Classes

JavaScript 提供两种 design patterns: the module pattern(使用闭包捕获和封装数据，ts形式化为namespace) and the class pattern(uses prototype chains). 
...
上面脚本，创建变量通过构造函数返回一个同名实例。If we were to write this type as an interface it would look like the following.
...
'new' 会调用 constructor. 函数类型可以同时有调用形式构造和构造函数.
...
The 'public' keyword 以为 retained as a field. 除了默认访问控制还有 private or protected 对于类成员. 静态类型检查会关注访问权限。
...
using the 'super' keyword 调用基类函数. 生成的 JavaScript code 之类会关联基类属性. 
...
Summarize a set of numeric constants as an *enum type*. 
...
the `const` modifier 用于生成js代码 improves performance on many JavaScript engines.
...
building a jump table indexed by case value 实现 switch。

## <a name="1.8"/>1.8 Overloading on String Parameters

TypeScript includes generic types, and *overloading on string parameters*.
...
*generic types* 准确捕获js库行为. type information to flow from client code, through library code, and back into client code, generic types may do more than any other TypeScript feature to support detailed API descriptions.
...
接口 have one or more *type parameters*. optional parameter 'compareFn'。
...
In TypeScript, classes can also have type parameters. 
...
TypeScript enforces *encapsulation* of implementation in classes at design time 但是运行时都可以访问。 

In JavaScript, 使用闭包支持运行时封装.
...
模块包括两个基本元素: a *module closure* and a *module* *object*. This *augmentation* 简化 dynamic loading of modules and also supports separation of module code into multiple files.

outer scope has assigned the module object to the variable 'MessageModule'.
...
```
If we were to describe the effect of namespace 'M' in terms of interfaces and variables, we would write
...
TypeScript type names and variable names do not conflict (see section [2.3](#2.3) for more details).
...
TypeScript files that contain top-level *export* and *import* directives. 生成 down-level 兼容代码.
